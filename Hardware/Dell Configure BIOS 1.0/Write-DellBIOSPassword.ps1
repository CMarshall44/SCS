Function Write-DellBIOSPassword {
<#
  .Synopsis 
  ...
  .Example
    $DellBIOSCredential = Write-DellBIOSPassword  
  .Example
    $SecureStringPwd = Read-Host -AsSecureString	
    Write-DellBIOSPassword –Password $SecureStringPwd
  .Example
    Write-DellBIOSPassword [[-FilePath] <String>] [[-Key] <String>] [[-Hint] <String>] [-Password] <SecureString> [<CommonParameters>]  
    #>

[CmdletBinding()]
Param (
  [Parameter(Mandatory=$false,ValueFromPipelineByPropertyName=$true,ValueFromPipeline=$true, position=0, HelpMessage="Path to Password File")] [string] $FilePath,
  [Parameter(Mandatory=$false,ValueFromPipelineByPropertyName=$true,ValueFromPipeline=$false, position=1, HelpMessage="An ASCII Key of 128,196 of 256 Length")] [byte []] $Key,    
  ...
  [Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true,ValueFromPipeline=$false, position=5, HelpMessage="DellBIOS Password")] [System.Security.SecureString] $Password
 ) 
...
PROCESS {
    ...
    {
        $FilePath = (Get-DellBIOSPasswordPath).DellBIOSPwdFilePath
    }
    ...
    [Xml]$xml="<Credential><Password></Password><Encryption>DAPI</Encryption></Credential>"
    ...
    {        
        $xml.Credential.Password= (ConvertFrom-SecureString -SecureString $Password -Key $Key).ToString()
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCqwat+URn/SR/y
# xPPlY1sNGQA0cA3NEFwI7hl1jsE9QqCCCscwggUyMIIEGqADAgECAg0Ah4JSYAAA
...
# 9w0BCQQxIgQgOVgIV1Y30cDV7ulXDzDWHN5YCZ7b85ygWfoK3X8CbRYwDQYJKoZI
# hvcNAQEBBQAEggEAlEjqQvo9YDilI0sqJBSSbgSyQ0tcoBu7Hzix3L32BDUm8JxD
# uBpkLFs7DFDJ8XqXEyUUAHasiKSFgmQNlTryoo8Y8ufOLOlYFT3OTQV6yD/58i+v
# XGlUYvOX5TjUvKoZzAOr9TLxM2OnzphPZaiN5WCk6A5rUqGxkKlI18Pi0y9m3NWS
# 7l6tPQDupcMeqnq/o18BVuj4YAOXwNIw2JWGTzVybZ6lqJFa24X+LqY9Nd187JNn
# n09HCTIarf4oGtM9TpxzTH1inGUV9SeJ+zPUDlHkheSZ5KHy59T+ZqGzViNy7YyE
# 2Z2EzlnyTuJMCiWJ1kiCXRofssoahZ2TjVF2m6GCDj0wgg45BgorBgEEAYI3AwMB
...
# AWUDBAIBBQAEIMcRDQAwdixLqYqjQKS3N9S2JOaBzAsjOiXbFLv/MSr9AhUAix+5
# fff9J9J8r2cop/1/Pp3xJkwYDzIwMjAwODI2MDY1MzE3WjADAgEeoIGGpIGDMIGA
...
# CRABBDAcBgkqhkiG9w0BCQUxDxcNMjAwODI2MDY1MzE3WjAvBgkqhkiG9w0BCQQx
# IgQgrWDI99tRpXu1/iY+/QOtkXp+ub3Zs/YA9VhrANRwAoUwNwYLKoZIhvcNAQkQ
# Ai8xKDAmMCQwIgQgxHTOdgB9AjlODaXk3nwUxoD54oIBPP72U+9dtx/fYfgwCwYJ
# KoZIhvcNAQEBBIIBAF/ennA6TypXn+ML5CXyrDTeiWfHVGJ9jNqFxYzdfuZ3cx7z
# 5FlqgTJXLOjvaypza+eWdUqAYqsTXqF+t0A6efaHbxbaWBxfNiTCPaYvWvBUX1It
# SIkbs8Km40EX2ckjNn1pc3GgcqWk31YDYmIr2QxhOBsYiXGJoqqC3GFk3mGMPNI1
# qEQ++N/zQ6i/KhkPzW0Lj5VHDxbVsia1byaH9XSQiOmHBIl/M2RxVl/jeq6u57WI
# X/SQHiaPbGdvF9Vd9w6mn5Ug3nv2kfh78/DEz+TuyEl0CD5yoVvozhD9lSAiju+b
# DWvLmC+6FIkVU8Xgiz72cV2+gGUYoUxkwc/CBFk=
